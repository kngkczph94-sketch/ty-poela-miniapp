...
begin
  ...
  -- Parse and validate every replacement row before touching the saved plan.
  perform 1
  ...
  if jsonb_array_length(p_items) = 0 then
    delete from public.meal_plans where user_id = v_user_id and plan_date = p_plan_date;
    return null;
  end if;

  insert into public.meal_plans(user_id, plan_date, source_ration_id)
  values (v_user_id, p_plan_date, p_source_ration_id)
  on conflict (user_id, plan_date) do update set source_ration_id = excluded.source_ration_id
  ...
  insert into public.meal_plan_items(
    ...
    custom_products, custom_calories, custom_protein_g, custom_fat_g, custom_carbs_g,
    planned_servings, portion_amount, portion_unit, custom_recipe_data, custom_image_url
  )
  select v_plan_id, item.*
  from jsonb_to_recordset(p_items) as item(
    meal_type public.meal_type, planned_recipe_id uuid, entry_source text,
    custom_title text, custom_products jsonb, custom_calories numeric,
    custom_protein_g numeric, custom_fat_g numeric, custom_carbs_g numeric,
