...
export function RecipeShareCard({ recipe, deepLink }: RecipeShareCardProps) {
  return (
    <article className="relative overflow-hidden rounded-[2rem] bg-white text-slate-950 shadow-2xl shadow-orange-200/70">
      ...
      <div className="relative min-h-48 bg-gradient-to-br from-orange-500 via-rose-400 to-amber-300 p-5 text-white">
        {recipe.imageUrl ? (
          <img
            alt={recipe.title}
        ...
        <div className="relative z-10 flex min-h-40 flex-col justify-between">
          <div className="flex items-center justify-between gap-3">
            <span className="rounded-full bg-white px-3 py-1 text-xs font-black uppercase tracking-wide text-orange-600 shadow-lg shadow-orange-900/10">
              Ты поела
            </span>
      ...
      <div className="relative p-5">
        <div className="grid grid-cols-4 gap-2 rounded-3xl bg-orange-50 p-3 text-center">
          {macroItems.map((item) => (
            <div className="rounded-2xl bg-white/70 px-2 py-3" key={item.key}>
              ...
              <p className="mt-0.5 text-[10px] font-extrabold uppercase tracking-wide text-slate-400">{item.label}</p>
            </div>
          ))}
        </div>

        <div className="mt-4 flex items-center justify-between gap-3 rounded-3xl bg-slate-950 p-4 text-white">
          <div>
        ...
        <a
          className="mt-4 flex items-center justify-center rounded-2xl bg-orange-500 px-4 py-3 text-center text-base font-black text-white shadow-lg shadow-orange-200 transition hover:bg-orange-600"
          href={deepLink}
        >
