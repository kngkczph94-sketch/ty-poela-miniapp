...
export function AiRecipeModal({ initialDay = 'Сегодня', initialSlot = 'breakfast', onClose, onChoose }: Props) {
  ...
  return <div aria-modal="true" className="ai-recipe-overlay" role="dialog">
    <section className="ai-recipe-modal">
      <header className="ai-recipe-header"><div><div className="ai-recipe-kicker">ИИ · подбор рецепта</div><h2 className="ai-recipe-title">Что приготовить?</h2></div><button aria-label="Закрыть" className="ai-recipe-close" onClick={onClose} type="button">×</button></header>
      <div className="ai-recipe-tabs"><button className={`ai-recipe-tab ${mode === 'products' ? 'is-active' : ''}`} onClick={() => { setMode('products'); setSuggestions([]); }} type="button">По продуктам</button><button className={`ai-recipe-tab ${mode === 'photo' ? 'is-active' : ''}`} onClick={() => { setMode('photo'); setSuggestions([]); }} type="button">По фото</button></div>
