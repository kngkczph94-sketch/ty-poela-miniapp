...
function HomePage({ subscriptionStatus, onOpenAccess, onOpenRations, onOpenRecipes, onOpenCart, onOpenProgress, onOpenMacros }: { subscriptionStatus: SubscriptionStatus; onOpenAccess: () => void; onOpenRations: () => void; onOpenRecipes: () => void; onOpenCart: () => void; onOpenProgress: () => void; onOpenMacros: () => void }) {
  const cards = [
    { title: 'Рационы дня', description: 'Готовый день питания из 4 приёмов пищи', icon: '🥣', onClick: onOpenRations, tone: 'bg-[#EEF1D8] text-[#686F12]' },
    { title: 'Рассчитать БЖУ', description: 'Калории, БЖУ и подгонка рациона', icon: '🧮', onClick: onOpenMacros, tone: 'bg-[#EEF1D8] text-[#686F12]' },
    { title: 'Рецепты', description: 'Тёплые блюда на каждый день', icon: '📖', onClick: onOpenRecipes, tone: 'bg-[#EEF1D8] text-[#686F12]' },
    { title: 'База знаний', description: 'Мягкие подсказки о питании', icon: '📚', soon: true, tone: 'bg-[#EEF1D8] text-[#686F12]' },
    { title: 'ИИ-подбор', description: 'Персональный подбор рецептов', icon: '✨', soon: true, tone: 'bg-[#EEF1D8] text-[#686F12]' },
    { title: 'Прогресс', description: 'Мягкий трекер без давления', icon: '🌷', onClick: onOpenProgress, tone: 'bg-[#EEF1D8] text-[#686F12]' },
  ];
  return <>
    <section className="relative overflow-hidden rounded-[2rem] border border-[#686F12]/20 bg-[#EEF1D8] p-6 text-[#30360E] shadow-soft"><div className="absolute -right-10 -top-10 h-36 w-36 rounded-full bg-[#FAF7EF]/50" /><div className="absolute -bottom-14 left-12 h-40 w-40 rounded-full bg-[#686F12]/10" /><div className="relative z-10"><p className="mb-3 inline-flex rounded-full bg-[#FAF7EF]/70 px-3 py-1 text-sm font-semibold backdrop-blur">Telegram Mini App</p><h1 className="max-w-xs text-4xl font-black leading-tight tracking-tight text-[#30360E]">Ты поела?</h1><p className="mt-4 max-w-sm text-base font-medium leading-6 text-[#92735C]">Рационы, рецепты и план питания внутри Telegram</p><button className="mt-6 rounded-2xl bg-[#686F12] px-5 py-3 text-base font-bold text-white shadow-lg shadow-[#E2D4B9]/70 transition hover:bg-[#30360E]" onClick={onOpenRations} type="button">Выбрать рацион дня</button></div></section>
    <section className="mt-5 rounded-[2rem] border border-[#686F12]/30 bg-[#F3F5E6] p-5 shadow-sm shadow-[#E2D4B9]/70"><p className="text-xs font-black uppercase tracking-[0.18em] text-[#686F12]">Главное</p><h2 className="mt-2 text-2xl font-black leading-tight text-[#30360E]">Рацион — это план, а не интуиция</h2><p className="mt-2 text-sm font-semibold leading-5 text-[#92735C]">Собери день питания, добавь его в План — и получи понятную корзину без лишней суеты.</p></section>
    <section className="mt-5 rounded-3xl border border-[#92735C]/35 bg-[#FFFDF8] p-4 shadow-sm shadow-[#E2D4B9]/70"><div className="flex items-center justify-between gap-3"><div><p className="text-xs font-black uppercase tracking-[0.18em] text-[#686F12]">Статус доступа</p><h2 className="mt-1 text-lg font-black text-[#30360E]">{subscriptionStatus === 'active' ? 'Доступ активен' : 'Бесплатный доступ'}</h2><p className="mt-1 text-sm font-semibold text-[#92735C]">{subscriptionStatus === 'active' ? 'Premium-рационы открыты' : 'Открой premium-рационы и рецепты'}</p></div>{subscriptionStatus === 'free' && <button className="shrink-0 rounded-2xl bg-[#686F12] px-4 py-3 text-sm font-black text-white transition hover:bg-[#30360E]" onClick={onOpenAccess} type="button">Открыть доступ</button>}</div></section>
    <section className="mt-6 grid gap-3">{cards.map((card) => {
      ...
      const className = `rounded-3xl border border-[#92735C]/35 bg-[#FFFDF8] p-4 text-left shadow-sm shadow-[#E2D4B9]/70 ${card.onClick ? 'cursor-pointer transition hover:-translate-y-0.5 hover:shadow-md' : ''}`;
    ...
    <section className="mt-7 grid grid-cols-2 gap-3"><button className="rounded-3xl bg-[#FFFDF8] p-4 text-left font-black text-[#30360E] shadow-sm shadow-[#E2D4B9]/70" onClick={onOpenRecipes} type="button">Открыть рецепты</button><button className="rounded-3xl bg-[#FFFDF8] p-4 text-left font-black text-[#30360E] shadow-sm shadow-[#E2D4B9]/70" onClick={onOpenCart} type="button">Корзина</button></section>
  </>;
}
function AccessPage({
  subscriptionStatus,
  ...
  return (
    <section className="flex flex-1 flex-col">
      ...
      {subscriptionStatus === 'active' && (
        <div className="mt-5 rounded-3xl border border-[#92735C]/35 bg-[#F3F5E6] p-5 text-[#30360E] shadow-sm shadow-[#E2D4B9]/70">
      ...
      <article className="mt-5 rounded-[2rem] bg-[#FFFDF8] p-5 shadow-xl shadow-[#E2D4B9]/70">
        ...
        <button
          className="mt-6 w-full rounded-2xl bg-[#686F12] px-4 py-3 text-base font-black text-white shadow-lg shadow-[#E2D4B9]/70 transition hover:bg-[#30360E]"
          onClick={onActivate}
          type="button"
        >
      ...
      <button
        className="mt-4 rounded-2xl bg-white px-4 py-3 text-base font-black text-[#30360E] shadow-sm shadow-[#E2D4B9]/70 transition hover:bg-[#E2D4B9]"
...
function App() {
  ...
  return <main className="min-h-screen bg-[#FAF7EF] text-[#30360E]"><div className="mx-auto flex min-h-screen w-full max-w-md flex-col px-4 pb-28 pt-5">
  ...
  </div><nav className="fixed inset-x-0 bottom-0 z-20 mx-auto max-w-md border-t border-[#92735C]/35 bg-[#FAF7EF]/95 px-4 pb-5 pt-3 shadow-2xl shadow-[#E2D4B9]/70 backdrop-blur"><div className="grid grid-cols-5 gap-1">{navigationItems.map((item)=><button className={`flex flex-col items-center gap-1 rounded-2xl px-1 py-2 text-[11px] font-bold transition ${activeTab === item.id && !showProgressCard ? 'bg-[#E9EDCF] text-[#686F12]' : 'text-[#92735C] hover:text-[#30360E]'}`} key={item.id} onClick={()=>{ setShowProgressCard(false); setActiveTab(item.id); setSelectedRecipe(null); setSelectedRation(null); }} type="button"><span className="text-lg">{item.icon}</span>{item.label}</button>)}</div></nav></main>;
}
