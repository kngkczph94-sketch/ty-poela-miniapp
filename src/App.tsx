...
function HomePage({ subscriptionStatus, onOpenAccess, onOpenRations, onOpenRecipes, onOpenCart, onOpenProgress, onOpenMacros }: { subscriptionStatus: SubscriptionStatus; onOpenAccess: () => void; onOpenRations: () => void; onOpenRecipes: () => void; onOpenCart: () => void; onOpenProgress: () => void; onOpenMacros: () => void }) {
  ...
  return <>
    <section className="relative overflow-hidden rounded-[2rem] border border-[#D99663]/45 bg-gradient-to-br from-[#F3E2BF] via-[#D99663]/45 to-[#FBF6EC] p-6 text-[#37410F] shadow-xl shadow-[#D99663]/25"><div className="absolute -right-10 -top-10 h-36 w-36 rounded-full bg-[#D99663]/20" /><div className="absolute -bottom-14 left-12 h-40 w-40 rounded-full bg-[#6E7E1F]/15" /><div className="relative z-10"><p className="mb-3 inline-flex rounded-full bg-[#FFFDF8]/80 px-3 py-1 text-sm font-semibold text-[#6E7E1F] backdrop-blur">Telegram Mini App</p><h1 className="max-w-xs text-4xl font-black leading-tight tracking-tight text-[#37410F]">Ты поела?</h1><p className="mt-4 max-w-sm text-base font-medium leading-6 text-[#8B725F]">Рационы, рецепты и план питания внутри Telegram</p><button className="mt-6 rounded-2xl bg-[#6E7E1F] px-5 py-3 text-base font-bold text-white shadow-lg shadow-[#6E7E1F]/25 transition hover:bg-[#37410F]" onClick={onOpenRations} type="button">Выбрать рацион дня</button></div></section>
    <section className="mt-5 overflow-hidden rounded-[2rem] border border-[#D99663]/40 bg-[#FFFDF8] p-4 shadow-lg shadow-[#F3E2BF]/80"><div className="grid gap-4 sm:grid-cols-[1fr_9rem] sm:items-center"><div className="p-1"><p className="text-xs font-black uppercase tracking-[0.18em] text-[#6E7E1F]">Главное</p><h2 className="mt-2 text-2xl font-black leading-tight text-[#37410F]">Рацион — это план, а не интуиция</h2><p className="mt-2 text-sm font-semibold leading-5 text-[#8B725F]">Собери день питания, добавь его в План — и получи понятную корзину без лишней суеты.</p></div><FoodPhotoPlaceholder className="min-h-[8rem]" variant="hero" /></div></section>
    ...
    <section className="mt-6 grid gap-3">{cards.map((card) => {
      ...
      const className = `rounded-3xl border border-[#D99663]/25 bg-[#FFFDF8] p-4 text-left shadow-sm shadow-[#F3E2BF]/70 ${card.onClick ? 'cursor-pointer transition hover:-translate-y-0.5 hover:shadow-md' : ''}`;
