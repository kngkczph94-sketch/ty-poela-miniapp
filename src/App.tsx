...
function HomePage({ subscriptionStatus, onOpenAccess, onOpenRations, onOpenRecipes, onOpenCart, onOpenProgress, onOpenMacros }: { subscriptionStatus: SubscriptionStatus; onOpenAccess: () => void; onOpenRations: () => void; onOpenRecipes: () => void; onOpenCart: () => void; onOpenProgress: () => void; onOpenMacros: () => void }) {
  ...
  return <>
    ...
    <section className="mt-5 rounded-[2rem] border border-ghee bg-butter p-5 shadow-sm shadow-butter"><p className="text-xs font-black uppercase tracking-[0.18em] text-olive-dark">Главное</p><h2 className="mt-2 text-2xl font-black leading-tight text-warm-dark">Рацион — это план, а не интуиция</h2><p className="mt-2 text-sm font-semibold leading-5 text-warm-dark/70">Собери день питания, добавь его в План — и получи понятную корзину без лишней суеты.</p></section>
    <section className="mt-5 rounded-3xl border border-butter bg-white p-4 shadow-sm shadow-butter"><div className="flex items-center justify-between gap-3"><div><p className="text-xs font-black uppercase tracking-[0.18em] text-olive">Статус доступа</p><h2 className="mt-1 text-lg font-black text-warm-dark">{subscriptionStatus === 'active' ? 'Доступ активен' : 'Бесплатный доступ'}</h2><p className="mt-1 text-sm font-semibold text-warm-dark/65">{subscriptionStatus === 'active' ? 'Premium-рационы открыты' : 'Открой premium-рационы и рецепты'}</p></div>{subscriptionStatus === 'free' && <button className="shrink-0 rounded-2xl bg-warm-dark px-4 py-3 text-sm font-black text-white" onClick={onOpenAccess} type="button">Открыть доступ</button>}</div></section>
    <section className="mt-6 grid gap-3">{cards.map((card) => {
      const content = <div className="flex items-start gap-4"><span className={`flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl text-2xl ${card.tone}`}>{card.icon}</span><div className="min-w-0 flex-1"><div className="flex items-center gap-2"><h2 className="text-lg font-extrabold text-warm-dark">{card.title}</h2>{card.soon && <span className="rounded-full bg-butter px-2 py-0.5 text-[10px] font-black uppercase tracking-wide text-olive-dark">скоро</span>}</div><p className="mt-1 text-sm leading-5 text-warm-dark/65">{card.description}</p></div></div>;
      const className = `rounded-3xl border border-butter bg-white p-4 text-left shadow-sm shadow-butter ${card.onClick ? 'cursor-pointer transition hover:-translate-y-0.5 hover:shadow-md' : ''}`;
    ...
    <section className="mt-7 grid grid-cols-2 gap-3"><button className="rounded-3xl bg-white p-4 text-left font-black text-olive-dark shadow-sm shadow-butter" onClick={onOpenRecipes} type="button">Открыть рецепты</button><button className="rounded-3xl bg-white p-4 text-left font-black text-olive-dark shadow-sm shadow-butter" onClick={onOpenCart} type="button">Корзина</button></section>
  </>;
}
function AccessPage({
  subscriptionStatus,
  ...
  return (
    <section className="flex flex-1 flex-col">
      <div className="overflow-hidden rounded-[2rem] bg-gradient-to-br from-warm-dark via-olive-dark to-ghee p-6 text-white shadow-xl shadow-ghee/70">
      ...
      <article className="mt-5 rounded-[2rem] bg-white p-5 shadow-xl shadow-butter">
        ...
        <div className="mt-5 space-y-3">
          {[
            ...
            <p className="flex items-center gap-3 text-sm font-bold text-warm-dark" key={item}>
              <span className="flex h-7 w-7 items-center justify-center rounded-full bg-butter-soft text-olive-dark">✓</span>
              {item}
            </p>
          ))}
        </div>
        ...
        <button
          className="mt-6 w-full rounded-2xl bg-olive px-4 py-3 text-base font-black text-white shadow-lg shadow-ghee transition hover:bg-olive-dark"
          onClick={onActivate}
          type="button"
        >
          {subscriptionStatus === 'active' ? 'Продлить mock-доступ' : 'Открыть полный доступ'}
        </button>
        <p className="mt-3 text-center text-xs font-bold text-warm-dark/45">Mock-режим: настоящие Telegram Stars пока не подключены.</p>
      </article>

      <button
        className="mt-4 rounded-2xl bg-white px-4 py-3 text-base font-black text-olive-dark shadow-sm shadow-butter transition hover:bg-butter-soft"
