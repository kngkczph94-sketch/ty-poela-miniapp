...
function HomePage({ subscriptionStatus, onOpenAccess, onOpenRations, onOpenRecipes, onOpenCart, onOpenProgress, onOpenMacros }: { subscriptionStatus: SubscriptionStatus; onOpenAccess: () => void; onOpenRations: () => void; onOpenRecipes: () => void; onOpenCart: () => void; onOpenProgress: () => void; onOpenMacros: () => void }) {
  ...
  return <>
    <section className="relative overflow-hidden rounded-[2rem] border border-[#D99663]/30 bg-[#F3E2BF] p-6 text-[#37410F] shadow-soft"><div className="absolute -right-10 -top-10 h-36 w-36 rounded-full bg-[#D99663]/20" /><div className="absolute -bottom-14 left-12 h-40 w-40 rounded-full bg-[#6E7E1F]/15" /><div className="relative z-10"><p className="mb-3 inline-flex rounded-full bg-[#FBF6EC]/70 px-3 py-1 text-sm font-semibold backdrop-blur">Telegram Mini App</p><h1 className="max-w-xs text-4xl font-black leading-tight tracking-tight text-[#37410F]">Ты поела?</h1><p className="mt-4 max-w-sm text-base font-medium leading-6 text-[#8B725F]">Рационы, рецепты и план питания внутри Telegram</p><button className="mt-6 rounded-2xl bg-[#6E7E1F] px-5 py-3 text-base font-bold text-white shadow-lg shadow-[#F3E2BF]/70 transition hover:bg-[#37410F]" onClick={onOpenRations} type="button">Выбрать рацион дня</button></div></section>
    <section className="mt-5 rounded-[2rem] border border-[#D99663]/30 bg-[#FFFDF8] p-5 shadow-sm shadow-[#F3E2BF]/70"><p className="text-xs font-black uppercase tracking-[0.18em] text-[#6E7E1F]">Главное</p><h2 className="mt-2 text-2xl font-black leading-tight text-[#37410F]">Рацион — это план, а не интуиция</h2><p className="mt-2 text-sm font-semibold leading-5 text-[#8B725F]">Собери день питания, добавь его в План — и получи понятную корзину без лишней суеты.</p></section>
    <section className="mt-5 rounded-3xl border border-[#F3E2BF] bg-[#FFFDF8] p-4 shadow-sm shadow-[#F3E2BF]/70"><div className="flex items-center justify-between gap-3"><div><p className="text-xs font-black uppercase tracking-[0.18em] text-[#6E7E1F]">Статус доступа</p><h2 className="mt-1 text-lg font-black text-[#37410F]">{subscriptionStatus === 'active' ? 'Доступ активен' : 'Бесплатный доступ'}</h2><p className="mt-1 text-sm font-semibold text-[#8B725F]">{subscriptionStatus === 'active' ? 'Premium-рационы открыты' : 'Открой premium-рационы и рецепты'}</p></div>{subscriptionStatus === 'free' && <button className="shrink-0 rounded-2xl bg-[#6E7E1F] px-4 py-3 text-sm font-black text-white transition hover:bg-[#37410F]" onClick={onOpenAccess} type="button">Открыть доступ</button>}</div></section>
    <section className="mt-6 grid gap-3">{cards.map((card) => {
      ...
      const className = `rounded-3xl border border-[#F3E2BF] bg-[#FFFDF8] p-4 text-left shadow-sm shadow-[#F3E2BF]/70 ${card.onClick ? 'cursor-pointer transition hover:-translate-y-0.5 hover:shadow-md' : ''}`;
...
function App() {
  ...
  return <main className="min-h-screen bg-[#FBF6EC] text-[#37410F]"><div className="mx-auto flex min-h-screen w-full max-w-md flex-col px-4 pb-28 pt-5">
  ...
  </div><nav className="fixed inset-x-0 bottom-0 z-20 mx-auto max-w-md border-t border-[#8B725F]/35 bg-[#FFFDF8]/95 px-4 pb-5 pt-3 shadow-2xl shadow-[#F3E2BF]/70 backdrop-blur"><div className="grid grid-cols-5 gap-1">{navigationItems.map((item)=><button className={`flex flex-col items-center gap-1 rounded-2xl px-1 py-2 text-[11px] font-bold transition ${activeTab === item.id && !showProgressCard ? 'bg-[#F3E2BF] text-[#6E7E1F]' : 'text-[#8B725F] hover:text-[#37410F]'}`} key={item.id} onClick={()=>{ setShowProgressCard(false); setActiveTab(item.id); setSelectedRecipe(null); setSelectedRation(null); }} type="button"><span className="text-lg">{item.icon}</span>{item.label}</button>)}</div></nav></main>;
}
