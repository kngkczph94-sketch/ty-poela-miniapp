...
function HomePage({ subscriptionStatus, onOpenAccess, onOpenRations, onOpenRecipes, onOpenCart, onOpenProgress, onOpenMacros }: { subscriptionStatus: SubscriptionStatus; onOpenAccess: () => void; onOpenRations: () => void; onOpenRecipes: () => void; onOpenCart: () => void; onOpenProgress: () => void; onOpenMacros: () => void }) {
  ...
  return <>
    <section className="relative overflow-hidden rounded-[2rem] bg-[#EEF1D8] p-6 text-[#30360E] shadow-soft"><div className="absolute -right-10 -top-10 h-36 w-36 rounded-full bg-[#FAF7EF]/50" /><div className="absolute -bottom-14 left-12 h-40 w-40 rounded-full bg-[#686F12]/10" /><div className="relative z-10"><p className="mb-3 inline-flex rounded-full bg-[#FAF7EF]/70 px-3 py-1 text-sm font-semibold backdrop-blur">Telegram Mini App</p><h1 className="max-w-xs text-4xl font-black leading-tight tracking-tight">Ты поела?</h1><p className="mt-4 max-w-sm text-base font-medium leading-6 text-[#92735C]">Рационы, рецепты и план питания внутри Telegram</p><button className="mt-6 rounded-2xl bg-[#686F12] px-5 py-3 text-base font-bold text-white shadow-lg shadow-[#E2D4B9]/70 transition hover:bg-[#30360E]" onClick={onOpenRations} type="button">Выбрать рацион дня</button></div></section>
