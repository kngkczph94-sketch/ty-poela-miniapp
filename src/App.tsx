...
function HomePage({ subscriptionStatus, onOpenAccess, onOpenRations, onOpenRecipes, onOpenCart, onOpenProgress, onOpenMacros }: { subscriptionStatus: SubscriptionStatus; onOpenAccess: () => void; onOpenRations: () => void; onOpenRecipes: () => void; onOpenCart: () => void; onOpenProgress: () => void; onOpenMacros: () => void }) {
  ...
  return <>
    ...
    <section className="mt-5 rounded-[2rem] border border-[#D99663]/40 bg-[#FFFDF8] p-5 shadow-lg shadow-[#F3E2BF]/80"><p className="text-xs font-black uppercase tracking-[0.18em] text-[#6E7E1F]">Главное</p><h2 className="mt-2 text-2xl font-black leading-tight text-[#37410F]">Рацион — это план, а не интуиция</h2><p className="mt-2 text-sm font-semibold leading-5 text-[#8B725F]">Собери день питания, добавь его в План — и получи понятную корзину без лишней суеты.</p></section>
