...
export function MenuPage({ weeklyMenu, onOpenCart, onOpenRations, onOpenRecipe, onRemoveRecipe }: MenuPageProps) {
  ...
  return <section className="flex flex-1 flex-col">
    ...
    <div className="mt-5 space-y-4">{menuDays.map((day) => {
      ...
      {isEmpty ? <div className="mt-3 rounded-3xl bg-[#F3E2BF]/50 p-4 text-center"><p className="text-sm font-semibold text-[#8B725F]">Пока пусто. Добавь рацион дня — и план соберётся сам.</p><button className="mt-3 rounded-2xl bg-[#6E7E1F] px-4 py-3 text-sm font-black text-white" onClick={onOpenRations} type="button">Выбрать рацион</button></div> : <><div className="mt-3 grid grid-cols-4 gap-2 rounded-3xl bg-[#FFFDF8] p-3 text-center"><div><p className="text-sm font-black">{totals.calories}</p><p className="text-[11px] font-bold text-[#8B725F]">ккал</p></div><div><p className="text-sm font-black">{totals.protein} г</p><p className="text-[11px] font-bold text-[#8B725F]">белки</p></div><div><p className="text-sm font-black">{totals.fat} г</p><p className="text-[11px] font-bold text-[#8B725F]">жиры</p></div><div><p className="text-sm font-black">{totals.carbs} г</p><p className="text-[11px] font-bold text-[#8B725F]">углеводы</p></div></div><div className="mt-3 space-y-2">{menuMealSlots.map((slot) => { const meal = planDay.meals[slot]; return <div className="rounded-3xl border border-[#8B725F]/35 bg-[#F3E2BF]/50 p-3" key={slot}><p className="text-xs font-black uppercase tracking-wide text-[#6E7E1F]">{menuSlotLabels[slot]}</p>{meal ? <div className="mt-2"><button className="text-left text-base font-black text-[#37410F]" onClick={() => onOpenRecipe(meal)} type="button">{meal.title}</button><div className="mt-2 flex flex-wrap gap-2 text-[11px] font-extrabold text-[#8B725F]"><span className="rounded-full bg-white px-2 py-1">{meal.calories} ккал</span><span className="rounded-full bg-white px-2 py-1">Б {meal.protein} г</span><span className="rounded-full bg-white px-2 py-1">Ж {meal.fat} г</span><span className="rounded-full bg-white px-2 py-1">У {meal.carbs} г</span></div><button className="mt-3 rounded-full bg-white px-3 py-2 text-xs font-black text-[#37410F] shadow-sm" onClick={() => onRemoveRecipe(day, slot)} type="button">Удалить из слота</button></div> : <button className="mt-2 text-left text-sm font-extrabold text-[#6E7E1F]" onClick={onOpenRations} type="button">Выбрать рацион</button>}</div>})}</div></>}
      </article>})}</div>
  </section>;
}
