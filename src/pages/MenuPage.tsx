...
function DaySummary({ meals }: { meals: WeeklyMenu[MenuDay]['meals'] }) {
  ...
  return <section className="mt-4 overflow-hidden rounded-3xl border border-[#D99663]/30 bg-white" aria-label="Итог дня">
    <div className="bg-[#37410F] px-4 py-4 text-white">
      <p className="text-xs font-black uppercase tracking-[0.16em] text-[#F3E2BF]">Итог дня</p>
      {hasMeals ? <div className="mt-3 grid grid-cols-4 gap-2">
        {nutritionItems.map(({ key, label, suffix }) => <div key={key}>
          ...
          <p className="mt-0.5 text-[10px] font-bold text-[#F3E2BF] sm:text-xs">{label}</p>
        </div>)}
      </div> : <div className="mt-3 flex items-center gap-3 rounded-2xl bg-white/10 p-3">
        <span className="text-2xl" aria-hidden="true">🍽️</span>
        <div><p className="text-sm font-black">За этот день пока ничего не внесено</p><p className="mt-0.5 text-xs font-semibold text-[#F3E2BF]">Добавьте продукты или блюдо — итог появится здесь.</p></div>
      </div>}
    </div>
    {hasMeals && <div className="divide-y divide-[#D99663]/20 px-4">
      {menuMealSlots.map((slot) => {
        const meal = meals[slot];
        return <div className="flex items-center justify-between gap-3 py-3" key={slot}>
          <div className="min-w-0"><p className="text-sm font-black text-[#37410F]">{menuSlotLabels[slot]}</p><p className="truncate text-[11px] font-semibold text-[#8B725F]">{meal?.title ?? 'Не добавлено'}</p></div>
          {meal ? <p className="shrink-0 text-right text-xs font-extrabold text-[#37410F]">{meal.calories} ккал<br /><span className="text-[10px] text-[#6E7E1F]">Б {meal.protein} · Ж {meal.fat} · У {meal.carbs}</span></p> : <span className="shrink-0 text-xs font-bold text-[#B8A99D]">—</span>}
        </div>;
      })}
    </div>}
  </section>;
