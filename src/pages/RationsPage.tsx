...
export function RationsPage({ hasActiveSubscription, onOpenAccess, onOpenRation }: { hasActiveSubscription: boolean; onOpenAccess: () => void; onOpenRation: (ration: DailyRation) => void }) {
  ...
  return <section className="flex flex-1 flex-col">
    ...
    <div className="mt-2 space-y-3">{rations.map((ration)=>{
      ...
      return <article className={`rounded-3xl border p-4 shadow-sm ${locked?'border-[#92735C]/35 bg-[#E2D4B9]':'border-[#92735C]/35 bg-[#FAF7EF] shadow-[#E2D4B9]'}`} key={ration.id}>
        ...
        <div className="mt-4 grid grid-cols-4 gap-2 rounded-2xl bg-[#F7F3E8] p-3 text-center text-xs font-bold"><div><b>{totals.calories}</b><p>ккал</p></div><div><b>{totals.protein} г</b><p>белки</p></div><div><b>{totals.fat} г</b><p>жиры</p></div><div><b>{totals.carbs} г</b><p>углеводы</p></div></div>
        <button className="mt-4 w-full rounded-2xl bg-[#686F12] px-4 py-3 text-base font-black text-white shadow-lg shadow-[#E2D4B9]" onClick={()=> locked ? onOpenAccess() : onOpenRation(ration)} type="button">{locked?'Оформить доступ':'Открыть'}</button>
      </article>})}</div>
  </section>;
}
