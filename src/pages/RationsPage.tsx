...
export function RationsPage({ hasActiveSubscription, onOpenAccess, onOpenRation }: { hasActiveSubscription: boolean; onOpenAccess: () => void; onOpenRation: (ration: DailyRation) => void }) {
  ...
  return <section className="flex flex-1 flex-col">
    ...
    <div className="sticky top-0 z-10 -mx-4 mt-5 bg-[#FFFDF8] px-4 pb-3 pt-1">
      ...
      <div className="mt-3 flex gap-2 overflow-x-auto pb-1">{(['all','free','premium'] as Filter[]).map((item)=><button className={`shrink-0 rounded-full px-4 py-2 text-sm font-extrabold ${filter===item?'bg-[#6E7E1F] text-white':'bg-white text-[#8B725F] shadow-sm shadow-[#F3E2BF]/70'}`} key={item} onClick={()=>setFilter(item)} type="button">{item==='all'?'Все':item==='free'?'Бесплатные':'Premium'}</button>)}</div>
    </div>
    <div className="mt-2 space-y-3">{rations.map((ration)=>{
      ...
      return <article className={`rounded-3xl border p-4 shadow-sm ${locked?'border-[#8B725F]/35 bg-[#F3E2BF]':'border-[#F3E2BF] bg-[#FFFDF8] shadow-[#F3E2BF]/70'}`} key={ration.id}>
        <div className="flex items-start justify-between gap-3"><div><div className="flex gap-2"><span className="rounded-full bg-[#F3E2BF] px-3 py-1 text-xs font-extrabold text-[#37410F]">Рацион №{ration.rationNumber}</span>{ration.isPremium&&<span className="rounded-full bg-[#D99663]/15 px-3 py-1 text-xs font-extrabold text-[#D99663]">Premium</span>}</div><h2 className="mt-2 text-xl font-black text-[#37410F]">{ration.title}</h2><p className="mt-1 text-sm leading-5 text-[#8B725F]">{ration.description}</p></div></div>
        <div className="mt-3 grid gap-2">{Object.entries(ration.meals).map(([slot, meal])=><p className="rounded-2xl bg-[#F3E2BF] px-3 py-2 text-sm font-bold text-[#37410F]" key={slot}><span className="text-[#37410F]">{menuSlotLabels[slot as keyof typeof menuSlotLabels]}:</span> {locked ? 'детали после доступа' : meal.title}</p>)}</div>
