  ...
  return <section className="flex flex-1 flex-col">
    ...
    <div className="mt-2 space-y-3">{rations.map((ration)=>{
      ...
      return <article className={`rounded-3xl border p-4 shadow-sm ${locked?'border-[#D99663]/35 bg-[#F3E2BF]':'border-[#D99663]/25 bg-[#FFFDF8] shadow-[#F3E2BF]/70'}`} key={ration.id}>
        <div className="grid gap-4 sm:grid-cols-[7.5rem_1fr] sm:items-start"><FoodPhotoPlaceholder className="min-h-[7.5rem]" variant="ration" /><div><div className="flex gap-2"><span className="rounded-full bg-[#F3E2BF] px-3 py-1 text-xs font-extrabold text-[#37410F]">Рацион №{ration.rationNumber}</span>{ration.isPremium&&<span className="rounded-full bg-[#D99663]/15 px-3 py-1 text-xs font-extrabold text-[#D99663]">Premium</span>}</div><h2 className="mt-2 text-xl font-black text-[#37410F]">{ration.title}</h2><p className="mt-1 text-sm leading-5 text-[#8B725F]">{ration.description}</p></div></div>
