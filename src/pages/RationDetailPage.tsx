...
export function RationDetailPage({ ration, onBack, onOpenAccess, onOpenRecipe, onAddRationToPlan, hasActiveSubscription }: { ration: DailyRation; hasActiveSubscription: boolean; onBack: () => void; onOpenAccess: () => void; onOpenRecipe: (meal: Meal) => void; onAddRationToPlan: (ration: DailyRation, meals: DailyRation['meals'], days: MenuDay[]) => void }) {
  ...
  return <section className="flex flex-1 flex-col">
    ...
    <article className="overflow-hidden rounded-[2rem] bg-white shadow-xl shadow-orange-100"><div className="bg-gradient-to-br from-orange-500 via-amber-400 to-yellow-300 p-6 text-white"><div className="flex gap-2"><span className="rounded-full bg-white/25 px-3 py-1 text-xs font-extrabold">Рацион №{ration.rationNumber}</span>{ration.isPremium&&<span className="rounded-full bg-white px-3 py-1 text-xs font-extrabold text-amber-700">Premium</span>}</div><h1 className="mt-3 text-3xl font-black">{ration.title}</h1>{adaptation && <span className="mt-3 inline-flex rounded-full bg-white px-3 py-1 text-xs font-extrabold text-orange-600">Подогнано под вашу норму</span>}<p className="mt-3 text-sm font-medium leading-6 text-white/90">{ration.description}</p></div>
    <div className="p-5">{adaptation && <div className="mb-4 rounded-3xl border border-orange-100 bg-orange-50 p-4"><p className="text-sm font-black text-slate-950">Подогнано под вашу норму</p><p className="mt-1 text-sm font-semibold text-slate-600">Исходная калорийность: {adaptation.originalCalories} ккал · новая: {totals.calories} ккал</p><p className="mt-1 text-sm font-semibold text-slate-600">Граммовки изменены на {changePercent > 0 ? '+' : ''}{changePercent}%</p><button className="mt-3 rounded-2xl bg-white px-4 py-3 text-sm font-black text-orange-600" onClick={() => { setMeals(adaptation.originalMeals); setIsOriginalRestored(true); setToast('Оригинальные граммовки возвращены'); }} type="button">Вернуть оригинал</button></div>}<div className="grid grid-cols-4 gap-2 rounded-3xl bg-orange-50 p-3 text-center"><div><p className="text-sm font-black">{totals.calories}</p><p className="text-[11px] font-bold text-slate-400">ккал</p></div><div><p className="text-sm font-black">{totals.protein} г</p><p className="text-[11px] font-bold text-slate-400">белки</p></div><div><p className="text-sm font-black">{totals.fat} г</p><p className="text-[11px] font-bold text-slate-400">жиры</p></div><div><p className="text-sm font-black">{totals.carbs} г</p><p className="text-[11px] font-bold text-slate-400">углеводы</p></div></div>
