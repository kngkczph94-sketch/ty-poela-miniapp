...
export function RationDetailPage({ ration, onBack, onOpenAccess, onOpenRecipe, onAddRationToPlan, hasActiveSubscription }: { ration: DailyRation; hasActiveSubscription: boolean; onBack: () => void; onOpenAccess: () => void; onOpenRecipe: (meal: Meal) => void; onAddRationToPlan: (ration: DailyRation, meals: DailyRation['meals'], days: MenuDay[]) => void }) {
  ...
  return <section className="flex flex-1 flex-col">
    ...
    <div className="mt-4 space-y-3">{Object.entries(meals).map(([slot, meal]) => <div className="rounded-3xl border border-[#8B725F]/35 bg-[#F3E2BF]/50 p-3" key={slot}><p className="text-xs font-black uppercase tracking-wide text-[#6E7E1F]">{menuSlotLabels[slot as MenuMealSlot]}</p><h3 className="mt-2 text-base font-black text-[#37410F]">{meal.title}</h3><p className="mt-1 text-sm text-[#8B725F]">{meal.description}</p><div className="mt-2 flex flex-wrap gap-2 text-[11px] font-extrabold text-[#8B725F]"><span className="rounded-full bg-white px-2 py-1">{meal.calories} ккал</span><span className="rounded-full bg-white px-2 py-1">Б {meal.protein} г</span><span className="rounded-full bg-white px-2 py-1">Ж {meal.fat} г</span><span className="rounded-full bg-white px-2 py-1">У {meal.carbs} г</span></div><div className="mt-3 flex gap-2"><button className="rounded-full bg-white px-3 py-2 text-xs font-black text-[#37410F]" onClick={()=>onOpenRecipe(meal)} type="button">Открыть</button><button className="rounded-full bg-[#6E7E1F] px-3 py-2 text-xs font-black text-white transition hover:bg-[#37410F]" onClick={()=>{setReplaceSlot(slot as MenuMealSlot); setShowAllMeals(false)}} type="button">Заменить</button></div></div>)}</div>
