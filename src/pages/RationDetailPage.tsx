...
export function RationDetailPage({ ration, onBack, onOpenAccess, onOpenRecipe, onAddRationToPlan, hasActiveSubscription }: { ration: DailyRation; hasActiveSubscription: boolean; onBack: () => void; onOpenAccess: () => void; onOpenRecipe: (meal: Meal) => void; onAddRationToPlan: (ration: DailyRation, meals: DailyRation['meals'], days: MenuDay[]) => Promise<void> }) {
  ...
  return <section className="flex flex-1 flex-col">
    ...
    <button className="mt-4 w-full rounded-2xl bg-[#6E7E1F] px-4 py-3 text-base font-black text-white shadow-lg shadow-[#F3E2BF]/70 disabled:cursor-wait disabled:opacity-60" disabled={isAddingToPlan} onClick={()=>setPickerOpen(!pickerOpen)} type="button">{isAddingToPlan ? 'Сохраняем…' : 'Добавить в план'}</button>
