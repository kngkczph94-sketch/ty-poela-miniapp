  ...
  return (
    <section className="flex flex-1 flex-col">
      ...
      <article className="mt-5 rounded-[2rem] bg-white p-5 shadow-sm shadow-butter">
        <h2 className="text-xl font-black text-warm-dark">Новый замер</h2>
        <form className="mt-4 space-y-4" onSubmit={handleSubmit} ref={formRef}>
          ...
          <button className="w-full rounded-2xl bg-olive px-4 py-3 text-base font-black text-white shadow-lg shadow-ghee transition hover:bg-olive-dark" type="submit">
            Добавить замер
          </button>
        </form>
      ...
      <article className="mt-5 rounded-[2rem] bg-white p-5 shadow-sm shadow-butter">
        <h2 className="text-xl font-black text-warm-dark">Динамика веса</h2>
      ...
      <section className="mt-5">
        <h2 className="text-xl font-black text-warm-dark">История замеров</h2>
        {sortedEntries.length === 0 ? (
          <div className="mt-3 rounded-[2rem] bg-white p-6 text-center shadow-sm shadow-butter">
            <p className="text-5xl">🌷</p>
            <p className="mt-3 text-lg font-black text-warm-dark">Добавь первый замер, чтобы видеть динамику</p>
            <p className="mt-2 text-sm font-semibold leading-5 text-warm-dark/65">Можно указать только то, что хочется. Остальное подождёт.</p>
            <button
              className="mt-5 rounded-2xl bg-olive px-5 py-3 text-base font-black text-white shadow-lg shadow-ghee transition hover:bg-olive-dark"
            ...
            {sortedEntries.map((entry) => (
              <article className="rounded-[2rem] bg-white p-4 shadow-sm shadow-butter" key={entry.id}>
                ...
                <div className="mt-3 flex flex-wrap gap-2 text-xs font-black text-warm-dark/80">
                  ...
                  {entry.waist !== undefined && <span className="rounded-full bg-butter-soft px-3 py-2">Талия {entry.waist} см</span>}
                  {entry.hips !== undefined && <span className="rounded-full bg-butter-soft px-3 py-2">Бедра {entry.hips} см</span>}
                  {entry.chest !== undefined && <span className="rounded-full bg-butter-soft px-3 py-2">Грудь {entry.chest} см</span>}
                </div>
                {entry.comment && <p className="mt-3 rounded-3xl bg-cream p-3 text-sm font-semibold leading-5 text-warm-dark/65">{entry.comment}</p>}
              </article>
            ))}
          </div>
        )}
