    ...
    <section className="flex flex-1 flex-col">
      ...
      <article className="mt-5 rounded-[2rem] bg-white p-5 shadow-sm shadow-orange-100">
        <div className="grid gap-4">
          <div>
            <p className="mb-2 text-sm font-black text-slate-700">Пол</p>
            <div className="grid grid-cols-2 gap-2">
              {(['female', 'male'] as Gender[]).map((value) => <button className={`rounded-2xl px-4 py-3 text-sm font-black ${gender === value ? 'bg-orange-500 text-white' : 'bg-orange-50 text-orange-600'}`} key={value} onClick={() => setGender(value)} type="button">{value === 'female' ? 'Женский' : 'Мужской'}</button>)}
            </div>
          </div>
          <div className="grid grid-cols-3 gap-3">
            <label className="text-sm font-black text-slate-700">Возраст<input className="mt-2 w-full rounded-2xl border border-orange-100 bg-orange-50 px-3 py-3 text-base font-bold outline-none focus:border-orange-300" min="1" onChange={(event) => setAge(event.target.value)} type="number" value={age} /></label>
            <label className="text-sm font-black text-slate-700">Рост, см<input className="mt-2 w-full rounded-2xl border border-orange-100 bg-orange-50 px-3 py-3 text-base font-bold outline-none focus:border-orange-300" min="1" onChange={(event) => setHeight(event.target.value)} type="number" value={height} /></label>
            <label className="text-sm font-black text-slate-700">Вес, кг<input className="mt-2 w-full rounded-2xl border border-orange-100 bg-orange-50 px-3 py-3 text-base font-bold outline-none focus:border-orange-300" min="1" onChange={(event) => setWeight(event.target.value)} type="number" value={weight} /></label>
          </div>
          <div>
            <p className="mb-2 text-sm font-black text-slate-700">Активность</p>
            <div className="grid grid-cols-3 gap-2">{(['low', 'moderate', 'high'] as Activity[]).map((value) => <button className={`rounded-2xl px-2 py-3 text-xs font-black ${activity === value ? 'bg-orange-500 text-white' : 'bg-orange-50 text-orange-600'}`} key={value} onClick={() => setActivity(value)} type="button">{value === 'low' ? 'Низкая' : value === 'moderate' ? 'Умеренная' : 'Высокая'}</button>)}</div>
          </div>
          <div>
            <p className="mb-2 text-sm font-black text-slate-700">Цель</p>
            <div className="grid grid-cols-3 gap-2">{(['loss', 'maintain', 'gain'] as Goal[]).map((value) => <button className={`rounded-2xl px-2 py-3 text-xs font-black ${goal === value ? 'bg-orange-500 text-white' : 'bg-orange-50 text-orange-600'}`} key={value} onClick={() => setGoal(value)} type="button">{value === 'loss' ? 'Снижение' : value === 'maintain' ? 'Поддержание' : 'Набор'}</button>)}</div>
          </div>
        </div>
        <button className="mt-5 w-full rounded-2xl bg-slate-950 px-4 py-3 text-base font-black text-white shadow-lg" onClick={handleCalculate} type="button">Рассчитать</button>
      </article>
      ...
      {result && <>
        <article className="mt-5 rounded-[2rem] bg-white p-5 shadow-sm shadow-orange-100">
          ...
          <p className="mt-3 text-xs font-semibold leading-5 text-slate-400">Белок рассчитан ориентировочно: {PROTEIN_GRAMS_PER_KG} г на кг веса.</p>
        </article>

        <section className="mt-5">
          <h2 className="text-xl font-black text-slate-950">Подходящие рационы</h2>
          {suitableRations.length === 0 ? <div className="mt-3 rounded-[2rem] bg-white p-6 text-center text-sm font-bold text-slate-500 shadow-sm shadow-orange-100">Пока нет подходящих рационов. Добавим их позже.</div> : <div className="mt-3 space-y-3">{suitableRations.map(({ ration, totals }) => <article className="rounded-[2rem] bg-white p-4 shadow-sm shadow-orange-100" key={ration.id}><div className="flex items-start justify-between gap-3"><div><h3 className="text-lg font-black text-slate-950">Рацион №{ration.rationNumber}</h3><p className="mt-1 text-sm font-bold text-slate-500">{formatDifference(result.calories, totals.calories)}</p></div><span className="rounded-2xl bg-orange-50 px-3 py-2 text-sm font-black text-orange-600">{totals.calories} ккал</span></div><div className="mt-3 flex flex-wrap gap-2 text-[11px] font-extrabold text-slate-500"><span className="rounded-full bg-orange-50 px-2 py-1">Б {totals.protein} г</span><span className="rounded-full bg-orange-50 px-2 py-1">Ж {totals.fat} г</span><span className="rounded-full bg-orange-50 px-2 py-1">У {totals.carbs} г</span></div>{warningRationId === ration.id && <div className="mt-3 rounded-3xl border border-amber-200 bg-amber-50 p-3"><p className="text-sm font-bold text-amber-800">Этот рацион сильно отличается от вашей нормы. Лучше выбрать рацион ближе по калорийности.</p><button className="mt-2 rounded-2xl bg-amber-500 px-4 py-2 text-sm font-black text-white" onClick={() => handleAdapt(ration, true)} type="button">Всё равно подогнать</button></div>}<div className="mt-4 grid grid-cols-2 gap-2"><button className="rounded-2xl bg-orange-50 px-4 py-3 text-sm font-black text-orange-600" onClick={() => onOpenRation(ration)} type="button">Открыть</button><button className="rounded-2xl bg-orange-500 px-4 py-3 text-sm font-black text-white" onClick={() => handleAdapt(ration)} type="button">Подогнать под меня</button></div></article>)}</div>}
        </section>
      </>}
    </section>
  );
