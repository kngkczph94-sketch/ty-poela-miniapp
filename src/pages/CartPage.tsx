  ...
  if (cartIngredients.length === 0) {
    return (
      <section className="flex flex-1 flex-col">
        ...
        <div className="mt-5 rounded-[2rem] bg-white p-6 text-center shadow-sm shadow-butter">
          <p className="text-5xl">🛒</p>
          <h2 className="mt-3 text-xl font-black text-warm-dark">Сначала добавь рацион в План</h2>
          <p className="mt-2 text-sm font-semibold leading-5 text-warm-dark/65">Выбери рацион дня, а мы автоматически сложим продукты по категориям.</p>
          <button className="mt-5 rounded-2xl bg-olive px-5 py-3 text-base font-black text-white shadow-lg shadow-ghee transition hover:bg-olive-dark" onClick={onOpenRecipes} type="button">
            Выбрать рацион
          </button>
        </div>
      </section>
  ...
  return (
    <section className="flex flex-1 flex-col">
      <div className="rounded-[2rem] bg-gradient-to-br from-olive via-olive-dark to-butter p-6 text-white shadow-xl shadow-ghee/70">
        ...
        <button className="mt-5 rounded-2xl bg-white px-5 py-3 text-base font-black text-olive-dark shadow-lg shadow-olive-dark/10 transition hover:-translate-y-0.5" onClick={handleCopyList} type="button">
          Скопировать список
        </button>
      </div>
      ...
      <div className="mt-5 space-y-4">
        {categoryOrder.map((category) => {
          ...
          return (
            <article className="rounded-[2rem] bg-white p-4 shadow-sm shadow-butter" key={category}>
              <h2 className="text-xl font-black text-warm-dark">{categoryLabels[category]}</h2>
              <div className="mt-3 space-y-2">
                {categoryIngredients.map((ingredient) => {
                  const isChecked = checkedIngredientIds.includes(ingredient.id);

                  return (
                    <label className={`flex items-center gap-3 rounded-3xl border p-3 transition ${isChecked ? 'border-cream bg-cream text-warm-dark/45' : 'border-butter-soft bg-butter-soft/50 text-warm-dark'}`} key={ingredient.id}>
                      <input checked={isChecked} className="h-5 w-5 rounded border-ghee accent-olive" onChange={() => toggleIngredient(ingredient.id)} type="checkbox" />
                      <span className={`flex-1 text-sm font-extrabold ${isChecked ? 'line-through' : ''}`}>{ingredient.name}</span>
                      <span className={`rounded-full px-3 py-1 text-xs font-black ${isChecked ? 'bg-white text-warm-dark/45' : 'bg-white text-olive-dark'}`}>{formatAmount(ingredient.amount)} {ingredient.unit}</span>
                    </label>
                  );
                })}
              </div>
      ...
      {copyMessage && <div className="fixed inset-x-4 bottom-28 z-30 mx-auto max-w-sm rounded-2xl bg-warm-dark px-4 py-3 text-center text-sm font-bold text-white shadow-2xl">{copyMessage}</div>}
    </section>
  );
}
